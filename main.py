...
# Configuração do banco SQLite (unchanged)
def init_db():
    with sqlite3.connect('stock Data.db') as conn:
        cursor = conn.cursor()
        cursor.execute('''
...
# Função para salvar dados no banco (unchanged)
...
# Função para carregar dados do banco (unchanged)
...
def scrape_stock():
    ...
    # Configurar proxy (opcional, substitua com suas credenciais de proxy)
    ...
    try:
        ...
        # Aguardar carregamento da página (ajuste o tempo conforme necessário)
        driver.implicitly_wait(10)

        # Verificar se há página de desafio do Cloudflare
        if "checking your browser" in driver.page_source.lower():
            logger.warning("Cloudflare challenge detectado, aguardando...")
            driver.implicitly_wait(10)  # Aguardar resolução do desafio

        # Obter HTML da página
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        driver.quit()  # Fechar o navegador

        logger.info(f"Tamanho do HTML: {len(driver.page_source)} caracteres")
